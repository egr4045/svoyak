...
class GameState {
  ...
  getInitialState(hostUser, options = {}) {
    return {
      ...
      // Режиссура голоса. Сервер хранит только РЕЖИМ — кто именно «держит слово», каждый клиент
      // вычисляет сам из уже разосланного стейта (отвечающий/исполнитель/дуэлянт/держатель
      // картошки), поэтому хуки в каждый хендлер не нужны:
      //   'auto'   — говорит держатель слова и ведущий, остальные глушат себе микрофон;
      //   'open'   — говорят все (обычный трёп между вопросами);
      //   'silent' — говорит только ведущий («тишина в студии»).
      // Жёстко заглушить чужой микрофон SDK не умеет (нет модераторского API) — это
      // самоглушение по команде: игрок технически может включиться обратно.
      voiceMode: 'auto',
      eventLog: [],
