...
class GameState {
  ...
  getInitialState(hostUser, options = {}) {
    return {
      ...
      // Режиссура громкости. Сервер хранит только РЕЖИМ; кто «держит слово», каждый клиент
      // вычисляет сам из уже разосланного стейта, поэтому хуки в каждый хендлер не нужны:
      //   'auto'      — во время ответа остальных слышно тише (отвечающего не перекрикивают);
      //   'open'      — никого не приглушаем;
      //   'host_only' — остальных слышно тише всё время (ведущий объясняет правила).
      // Это ПРИГЛУШЕНИЕ чужого воспроизведения у слушателя, а не мьют: микрофоны не трогаем,
      // смех и реакции остаются фоном. См. src/composables/useVoiceFloor.js.
      voiceMode: 'auto',
      eventLog: [],
